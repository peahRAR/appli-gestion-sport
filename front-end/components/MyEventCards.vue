<template>
  ...
  <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    ">
        ...
        <div class="flex flex-col justify-between mt-4">
          <button @click="participate(event, true)" :disabled="event.isParticipating"
    ...
    'opacity-50 cursor-not-allowed': event.isParticipating,
          ...
          <button @click="participate(event, false)" :disabled="!event.isParticipating"
            class="bg-red-500 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" :class="{
    'opacity-50 cursor-not-allowed': !event.isParticipating,
  }">
            Je ne participe pas
          </button>
      ...
      (userRole === 1 || userRole === 2),
        ...
        <div>
          <span v-if="userRole === 1 || userRole === 2" @click="openDetailsModal(participant)"
            class="cursor-pointer capitalize">{{ participant.name }}</span>
  ...
  <TheModal :isOpen="showDetailsModal" title="Détails de l'utilisateur" @close="closeDetailsModal">
    <ul>
      <li>
        <NuxtImg v-if="userDetails.avatar" :src="userDetails.avatar.data" alt="Avatar"
